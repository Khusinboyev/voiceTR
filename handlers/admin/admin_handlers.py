...
@dp.message_handler(commands='panel', user_id=[boshAdmin, twoAdmin])
async def panel(message: types.Message):
	Markup = await main_menu()
	await message.answer('salom admin', reply_markup=Markup)


@dp.message_handler(lambda message: message.text == '🔙 Orqaga qaytish', state=Form.all_states, user_id=[boshAdmin, twoAdmin])
...
@dp.message_handler(lambda message: message.text == '🔙 Orqaga qaytish', user_id=[boshAdmin, twoAdmin])
...
@dp.message_handler(text="📊 statistika", user_id=[boshAdmin, twoAdmin])
...
@dp.message_handler(lambda message: message.text == "📌 Kanallar", user_id=[boshAdmin, twoAdmin])
...
@dp.message_handler(lambda message: message.text == "➕   Kanal qo`shish", state=Form.channel, user_id=[boshAdmin, twoAdmin])
...
@dp.message_handler(state=Form.channel_add2, user_id=[boshAdmin, twoAdmin])
...
@dp.message_handler(lambda message: message.text == "🚫  Kanal olib tashlash", user_id=[boshAdmin, twoAdmin])
async def channel_remove1(message: types.Message):
	markup = await back_btn()
	await message.answer('Kanal Userini @chanel_id formatida kiritng', reply_markup=markup)
	await Form.ChannelRemove.set()

@dp.message_handler(state=Form.ChannelRemove, user_id=[boshAdmin, twoAdmin])
...
@dp.message_handler(lambda message: message.text == "📜 Kanallar ro'yhati", state=Form.channel, user_id=[boshAdmin, twoAdmin])
...
@dp.message_handler(lambda message: message.text == '✍️ Xabar yuborish', user_id=[boshAdmin, twoAdmin])
async def send_message(message: types.Message):
	Markup = await send_message_btn()
	await message.answer("Menyudan tanlang", reply_markup=Markup)


@dp.message_handler(lambda message: message.text == 'Bitta foydalanuvchiga yuborish', user_id=[boshAdmin, twoAdmin])
...
@dp.message_handler(state=Form.OneUser, user_id=[boshAdmin, twoAdmin])
...
@dp.message_handler(state=Form.OneUser2, content_types=ContentType.ANY, user_id=[boshAdmin, twoAdmin])
async def one_user(message: types.Message, state: FSMContext):
	Markup = await main_menu()
	async with state.proxy() as data:
		id = data['OneUser']
...
@dp.message_handler(lambda message: message.text == 'Birnechta foydalanuchilarga yuborish', user_id=[boshAdmin, twoAdmin])
...
@dp.message_handler(state=Form.ManyUser, user_id=[boshAdmin, twoAdmin])
...
@dp.message_handler(state=Form.ManyUser2, content_types=ContentType.ANY, user_id=[boshAdmin, twoAdmin])
async def send_many2(message: types.Message, state: FSMContext):
	await state.finish()
	async with state.proxy() as data:
...
@dp.message_handler(lambda message: message.text == "Barcha foydalanuchilarga yuborish", user_id=[boshAdmin, twoAdmin])
...
@dp.message_handler(state=Form.AllUsers, content_types=ContentType.ANY, user_id=[boshAdmin, twoAdmin])
...
@dp.message_handler(lambda message: message.text == '♻️ Tozalash', user_id=[boshAdmin, twoAdmin])
...
@dp.message_handler(state=Form.ClearState, user_id=[boshAdmin, twoAdmin])
...
@dp.message_handler(is_reply=True, user_id=[boshAdmin, twoAdmin])
